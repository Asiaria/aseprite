...
{
  if (t > 0.00885645171)
    return pow(t, 0.3333333333333333);
  else
    return (t / 0.12841855 + 0.137931034);
}

// Auxiliary function for findBestfit3()
void rgbToOtherSpace(double& r, double& g, double& b, FitCriteria fc)
{
  if (fc == RGB)
    return;
  double Rl, Gl, Bl;
...
int Palette::findBestfit3(int r, int g, int b, int a, int mask_index) const
{
  ASSERT(r >= 0 && r <= 255);
  ASSERT(g >= 0 && g <= 255);
  ASSERT(b >= 0 && b <= 255);
  ...
  FitCriteria fc = OLD;
  if (fc == OLD) {
    return findBestfit(r, g, b, a, mask_index);
  }
